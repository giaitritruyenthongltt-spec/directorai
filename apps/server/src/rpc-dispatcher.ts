...
const handlers: Record<string, RpcHandler> = {
  'project.get': {
    schema: Empty,
    run: (_p, a) => a.getProject(),
  },
  'project.listSequences': {
    schema: Empty,
    run: (_p, a) => a.listSequences(),
  },
  ...
  'effect.apply': {
    ...
    run: (p, a) => {
      const params = p as { clipId: string; effectMatchName: string };
      return a.applyEffect(params);
    },
  },
  'effect.remove': {
